    ...
    def terminate(self):
        if self.pid:
            try:
                ...
                logging.error(f"Failed to kill pid {self.pid}: {str(e)}")
...
class SessionManager(object):
    ...
    def remove_debug_session_by_pid(self, gdbpid: int) -> List[str]:
        ...
        if debug_session:
            ...
            logging.info(f"could not find debug session with gdb pid {gdbpid}")
            orphaned_client_ids = []
        return orphaned_client_ids

    def remove_debug_session(self, debug_session: DebugSession) -> List[str]:
        logging.info(f"Removing debug session for pid {debug_session.pid}")
        try:
